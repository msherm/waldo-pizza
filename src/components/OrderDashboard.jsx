...
const OrderDashboard = (props) => {
  ...
  const pizzaCart = pizzas.length ? <div><h3>Pizza Cart</h3><ul className="order-pizzas">{ pizzas }</ul><div className="pizza-price">Total ${ calculateTotalCost(props.pizzaData, props.orders).toFixed(2) }</div><button className="place-order-button">Place Order</button></div> : null;
